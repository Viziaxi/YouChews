...
const API_BASE_URL = "https://youchews.onrender.com";
...
const AdminQueuePage: React.FC = () => {
  ...
  const handleManageQueue = async (type: "approve" | "deny") => {
    ...
    try {
      ...
      const denied_list =
        type === "deny" ? Array.from(selectedItems).map((id) => ({ id })) : [];
      ...
      if (response.data.status === 200) {
        setSuccessMessage(
          response.data.message ||
            `Successfully ${type}d ${selectedItems.size} item(s)`
        );
        setSelectedItems(new Set());
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="flex gap-4">
            <button
              onClick={() =>
                viewMode === "queue" ? fetchQueue() : fetchAllRestaurants()
              }
        ...
        {viewMode === "queue" && queueItems.length > 0 && (
          <div className="mb-6 bg-white rounded-lg shadow-md p-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              >
                {selectedItems.size === queueItems.length
                  ? "Deselect All"
                  : "Select All"}
              </button>
